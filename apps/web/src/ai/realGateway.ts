/**
 * The real {@link AiGateway}: the Eat tab's three prompt tasks, wired to the
 ...
 * the panels speak and the options objects those tasks take, and turns a refusal
 * or an empty result into the one error every panel already handles.
 *
 ...
 * path stays exactly the path the panels were built and tested against.
 */
...
export interface RealGatewaySettings {
   ...
   * built. A tab left open across midnight would otherwise plan meals starting
   * yesterday.
   */
  today: () => LocalDate;
}
...
/** The panels' free-text constraints, as `packages/ai` wants them. */
 ...
 * panel's manual path is the right answer either way, so it reads as the same
 * unavailable state with the model's own explanation.
 */
function refused(task: AiGatewayTask, what: string, reason: string | null): AiUnavailableError {
...
/**
 * Builds the gateway the Eat panels use when the coach is connected. Wrapped in
 * {@link validating}, so a malformed model response fails at this boundary
 * rather than writing junk into `food_items` (DESIGN.md §11).
 *
 * Returns {@link unavailableGateway} when there is no key (`client` is null) or
 * the browser is offline, which is what makes "connect the coach in Settings"
 * and "you are offline" the same, already-tested panel state.
 */
  ...
  return validating({
    ...
    async parseFood(input: ParseFoodInput) {
      ...
      if (result.items.length === 0) {
        throw refused('parseFood', 'read that', result.refusal?.message ?? null);
      }
      return result.items;
    },
    ...
    async generateRecipe(input: GenerateRecipeInput): Promise<Recipe> {
      ...
      if (result.recipe == null) {
        throw refused('generateRecipe', 'write a recipe', result.refusal?.message ?? null);
      }
      // `toRecipeDraft` gives the repository's draft shape; the panel renders a
    ...
    async generateMealPlan(input: GenerateMealPlanInput) {
      ...
      if (result.plan.length === 0) {
        throw refused('generateMealPlan', 'build a plan', result.refusal?.message ?? null);
      }
      return result.plan;
    },
  });
